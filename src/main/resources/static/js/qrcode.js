...
(function () {
	...
	var Drawing = useSVG ? svgDrawer : !_isSupportCanvas() ? (function () {
		...
		function drawRoundRect(ctx, x, y, width, height, radius, lineWidth, lineColor, backgroundColor) {
		...
		Drawing.prototype.addIcon = async function (iconSrc) {
            ...
            drawRoundRect(this._oContext, x, y, width, height, this._htOption.iconRadius, this._htOption.iconBorderWidth, this._htOption.iconBorderColor, this._htOption.iconBackColor);
			this._oContext.save();
			this._oContext.clip();
		...
		Drawing.prototype.addCurtain = async function (curtainSrc) {
			let map = new Map(imgCache);
			if(map.get(curtainSrc) === undefined){
				const image = await new Promise((resolve, reject) => {
					const image = new Image();
					image.crossOrigin="anonymous";
					image.src = curtainSrc;
					image.onload=()=>{
						imgCache.push([curtainSrc, image]);
						resolve(image);
					};
				});
				this._oContext.drawImage(image, 0, 0, this._htOption.curtainWidth, this._htOption.curtainHeight - 20);
			}else{
				const image = map.get(curtainSrc);
				this._oContext.drawImage(image, 0, 0, this._htOption.curtainWidth, this._htOption.curtainHeight - 20);
			}
		}		
		// 画最下方备注
		Drawing.prototype.addSignText = async function (text) {
			this._oContext.fillStyle = "#fff";
			this._oContext.fillRect(0, this._htOption.curtainHeight - 20, this._htOption.curtainWidth, 20);
			this._oContext.strokeStyle = "#000";
			this._oContext.font="14px Arial";
			this._oContext.textAlign="center";
			this._oContext.textBaseline="bottom";
			this._oContext.fillStyle = "#000";
			this._oContext.fillText(text, this._htOption.curtainWidth/2, this._htOption.curtainHeight - 2);
			this._oContext.save();
		}	
		 ...
		 * @param {Number} nNumber
		 */
		Drawing.prototype.toImg = function () {
			return this._oContext.canvas.toDataURL("image/jpeg");
		};
	...
	QRCode.prototype.makeCode = async function (sText) {
		...
		const signTextExists = typeof(this._htOption.signText) !== "undefined";
		...
		if(signTextExists) {
			this._oDrawing.addSignText(this._htOption.signText);
		}
		if(curtainImgExists) {
